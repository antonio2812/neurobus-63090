...
import { ArrowLeft, Loader2, Send, Image, Download } from "lucide-react";
...
const ImageGeneratorChat = ({ onBack }: ImageGeneratorChatProps) => {
  ...
  const initialMessage: ChatMessage = {
    id: 0,
    sender: 'ai',
    content: `Olá! Sou o **Gerador de Imagens com IA** da LucraAI. Descreva o produto ou a cena que você deseja criar para o seu anúncio.`,
  };
  
  const tipMessage: ChatMessage = {
    id: 0.1,
    sender: 'ai',
    content: `<span class="text-foreground font-bold">Dica:</span> Seja detalhado! Inclua cores, estilo (ex: 'fundo branco', 'iluminação suave') e o tipo de produto.`,
  };
  ...
  const ImageDisplay = ({ imageSrc }: { imageSrc: string }) => {
    const handleDownload = () => {
      ...
      link.href = imageSrc;
      link.download = `lucraai_image_${Date.now()}.jpeg`;
  ...
  const handleGenerateImage = async (prompt: string) => {
    ...
    setMessages((prev) => [
      ...prev,
      {
        id: loadingMessageId,
        sender: 'ai',
        content: `Processando prompt... A IA está criando sua imagem para: **${prompt}**...`,
        isTyping: true,
      },
    ]);
    ...
    <div className="flex flex-col h-[80vh] max-h-[600px] bg-background rounded-lg border border-border/50">
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            >
              ...
              {msg.imageSrc && <ImageDisplay imageSrc={msg.imageSrc} />}
            </div>
          </div>
        ))}
        <div ref={chatEndRef} />
      ...
      <div className="p-4 border-t border-border">
        <div className="flex items-center w-full bg-card rounded-3xl border border-border/50 shadow-lg">
          ...
          <Input
            placeholder={step === 'done' ? "Geração concluída. Clique em 'Gerar Nova Imagem'." : "Descreva a imagem que você deseja criar..."}
            value={input}
