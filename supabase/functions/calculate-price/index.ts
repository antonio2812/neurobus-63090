...
const calculateShopeePrice = (productCost: number, desiredMarginRate: number): MarketplaceCalculation => {
    ...
    return { finalIdealSalePrice: initialPrice, commissionRate, fixedFee, freightFee, commissionLimit: false };
};
...
serve(async (req) => {
  ...
  if (!GOOGLE_GEMINI_API_KEY && !OPENROUTER_API_KEY) {
    console.warn('Aviso: Nenhuma chave de API (OPENROUTER_API_KEY_ ou Gemini) está definida. A explicação da IA pode falhar.');
  }
